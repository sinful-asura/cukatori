 ...
 * upstream bills per key and sends no CORS headers, so the frontend goes through
...
export class MuscleWikiClient {
  ...
  private readonly apiKey =
    process.env.MUSCLEWIKI_API_KEY?.trim() ||
    process.env.MUSCLE_WIKI_API_KEY?.trim() ||
    '';
  ...
  async get<T>(path: string, params: Record<string, string | number | undefined>): Promise<T> {
    ...
    const response = await fetch(url, {
      headers: {
        ...
        'User-Agent': 'AscendOS/1.0 (exercise-library)',
      },
