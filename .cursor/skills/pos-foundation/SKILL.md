...
5. Install in `api/`: `@mikro-orm/core` `@mikro-orm/nestjs` `@mikro-orm/postgresql` `@mikro-orm/migrations` `@mikro-orm/seeder` `@mikro-orm/reflection` `@nestjs/jwt` `@nestjs/config` `bcryptjs` `cookie-parser` `class-validator` `class-transformer`. Types as needed.
...
9. Modules: `auth`, `users`. JWT httpOnly cookie. `POST /api/auth/login|register|logout`, `GET /api/auth/me`.
