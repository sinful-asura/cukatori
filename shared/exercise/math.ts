import { MUSCLE_LABELS, type MuscleMixItem, type MuscleSlug } from './types';

/** Epley-style estimate used across catalog logging and PR detection. */
...
export function muscleLabel(slug: string): string {
  return MUSCLE_LABELS[slug as MuscleSlug] ?? slug.replace(/-/g, ' ');
}

export function buildMuscleMix(
  contributions: Array<{ muscle: MuscleSlug; volumeKg: number }>,
): MuscleMixItem[] {
